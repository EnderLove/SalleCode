...
void BST::levelOrderRec(Node *root, int level, std::vector<std::vector<int>> &res){
...
void BST::sumLevelRec(Node *root, int level, std::vector<int> &sum){
    if (root == nullptr) return;
...
void BST::insert (T data) { root = insert (root, data); } 
void BST::delNode(T data) { root = delNode(root, data); } 
...
void BST::inTravOrder(Node *node){
    if (node != nullptr){
